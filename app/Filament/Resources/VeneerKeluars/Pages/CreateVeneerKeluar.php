...
class CreateVeneerKeluar extends Page
{
    protected static string $resource = VeneerKeluarResource::class;

    public function getView(): string
    {
        return 'filament.resources.veneer-keluars.pages.create-veneer-keluar';
    }
    ...
    /* Auto-save header & items to session on any property change */
    ...
    {
        if (!$this->item_tipe_veneer || !$this->item_id_jenis_kayu) return [];
    ...
    {
        if (!$this->item_tipe_veneer || !$this->item_id_jenis_kayu || !$this->item_kw) return [];

        if ($this->item_tipe_veneer === 'basah') {
            $summaries = HppVeneerBasahSummary::where('id_jenis_kayu', $this->item_id_jenis_kayu)
                ->where('kw', $this->item_kw)->where('stok_lembar', '>', 0)
                ->get(['panjang', 'lebar', 'tebal']);

            $opts = [];
            foreach ($summaries as $s) {
                $u = Ukuran::where('panjang', $s->panjang)->where('lebar', $s->lebar)
                    ->where('tebal', $s->tebal)->first();
                if ($u) $opts[$u->id] = $u->dimensi;
            }
            return $opts;
        } else {
    ...
    public function tambahBarang(): void
    {
        ...
        if ($existingIndex !== null) {
            // Produk sama → tambahkan qty, validasi total tidak melebihi stok
            $totalQty = $this->items[$existingIndex]['qty'] + $this->item_qty;
            if ($totalQty > $this->stok_sistem) {
                $this->addError('item_qty', "Total qty ({$totalQty} lbr) melebihi stok tersedia ({$this->stok_sistem} lbr).");
                return;
            }
            $this->items[$existingIndex]['qty'] = $totalQty;
        } else {
            // Produk baru → tambah row baru
            if ($this->item_qty > $this->stok_sistem) {
                $this->addError('item_qty', "Jumlah keluar ({$this->item_qty} lbr) melebihi stok tersedia ({$this->stok_sistem} lbr).");
                return;
            }
            $this->items[] = [
    ...
    private function saveDocument(string $status): void
    {
        $this->validate([
            ...
            'no_nota'     => 'required|string|unique:nota_barang_keluar,no_nota',
