            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full text-[10px] border-collapse border border-zinc-500 dark:border-zinc-800">
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        @php
                        $dateGroup = collect($items)->groupBy('tanggal');
                        @endphp
                        ...
                        <tr class="text-center align-middle font-medium text-zinc-900 dark:text-zinc-200">
                            ...
                            <td class="p-2 border border-zinc-500">{{ number_format($row['kw5'] ?? 0, 0) }}</td>
                            ...
                            {{-- MERGE: Kolektif (Pekerja, Harga, Ongkos-ongkos) --}}
                            @if($index === 0)
                            ...
                            <td class="p-2 border border-zinc-500 text-right font-black" rowspan="{{ $rowCount }}">{{ number_format($first['ongkos_per_m3'], 0, ',', '.') }}</td>
                            <td class="p-2 border border-zinc-500 text-right" rowspan="{{ $rowCount }}">{{ number_format($first['ongkos_mesin'], 0, ',', '.') }}</td>
                            <td class="p-2 border border-zinc-500 text-right font-bold text-orange-600" rowspan="{{ $rowCount }}">{{ number_format($first['ongkos_m3_mesin'], 0, ',', '.') }}</td>
                            <td class="p-2 border border-zinc-500 text-right font-bold text-orange-600" rowspan="{{ $rowCount }}">{{ number_format($first['ongkos_per_lb'], 0, ',', '.') }}</td>
                            <td class="p-2 border border-zinc-500 text-[9px] text-red-500 font-black italic" rowspan="{{ $rowCount }}">{{ $first['ket'] }}</td>
                            @endif
                        </tr>
                        @endforeach
                        @endforeach
